...
6. [Host Profile Format](#host-profile-format)
...
- Any host profile bindings (those remain in Host Profile)
...
**Required fields for composition (Normative):** `schema`, `[nak].id`, `[nak].version`, and `[paths].root` MUST be present per Presence semantics. Missing or empty values MUST emit `nak_pin_invalid` and mark the NAK as unresolved (no CriticalError).
...
- If `[trust]` is absent, state MUST be treated as "unknown", NAH MUST emit `trust_state_unknown` (subject to profile warning policy), and `trust_state_stale` MUST NOT be emitted.
...
- NAK selection policy and allow/deny rules
- Optional mapped bindings to specific NAK records
- Default environment values
- Warning policy actions
- Capability enforcement mappings
- Override policy and allowlist

**MUST NOT contain:**

- App-specific identity, versioning, or entrypoints
- Per-version NAK paths as the primary resolution mechanism
- Trust decisions or verification outputs

**Canonical NAK namespace (Normative):**

- `[nak]` is the ONLY canonical configuration namespace for NAK selection policy.
- `[nak.map]` is the canonical mapping table for mapped mode.
- `[nak]` MUST contain only `binding_mode`, `allow_versions`, `deny_versions`, and optional `[nak.map]` for mapped mode; the manifest `nak_id` is authoritative and the Host Profile MUST NOT define or override required NAK identity.
...
- The authoritative source of app installation state is the set of App Install Records in `<nah_root>/registry/installs/`
...
- Registry updates MUST be atomic using the same procedure defined for profile updates (temp + fsync + rename + fsync directory)

### Atomic Update Requirements

All updates to host profiles MUST be atomic:
...
### Host Profile Responsibilities (Normative)

The Host Profile captures host-owned policy and bindings that an app cannot know:

- Which NAK versions are allowed (allow/deny patterns + binding_mode rules)
- How NAK versions are selected (binding rules)
- Warning policy (warn/ignore/error) for known conditions
- Capability-to-enforcement mapping (enforcement IDs only; host implements enforcement)

The Host Profile MUST NOT contain:

- App-specific identity, versioning, or entrypoints
- NAK installation roots (registry records are authoritative)
- The app-required NAK identity; the manifest `nak_id` is authoritative and the Host Profile MUST NOT define or override required NAK identity

### Active Host Profile Resolution (Normative)

NAH MUST resolve the active Host Profile as follows:
...
- Version patterns are prefix patterns ending in `*` (e.g., `3.*`, `3.1.*`) or exact matches.
- Deny rules take precedence over allow rules.
- If `allow_versions` is empty, all versions are allowed unless denied.

**Override Policy (Normative):**

- If `mode = "deny"`, all overrides MUST be ignored and each encountered override MUST emit `override_denied`.
- If `mode = "allow"`, ONLY the standardized override targets are permitted: `NAH_OVERRIDE_ENVIRONMENT` and `NAH_OVERRIDE_WARNINGS_<warning_key>`. `allow_keys` MUST be ignored in this mode.
- If `mode = "allowlist"`, permitted override targets are EXACTLY those matched by `allow_keys` per the allowlist matching rules within the supported target set. The standardized targets are not special-cased and are permitted ONLY if allowlisted.
- Supported override targets in v1.0 are only `NAH_OVERRIDE_ENVIRONMENT` and `NAH_OVERRIDE_WARNINGS_<warning_key>`. Any other `NAH_OVERRIDE_<X>` MUST be treated as not permitted, MUST emit `override_denied`, and MUST NOT emit `override_invalid`.

**Override Allowlist Matching (Normative):**

- The target key is the string after stripping `NAH_OVERRIDE_`.
- Matching rules:
  - Exact match (e.g., `ENVIRONMENT`)
  - Prefix wildcard ending in `*` (e.g., `WARNINGS_*` matches any target starting with `WARNINGS_`).
- The standardized override targets are `ENVIRONMENT` and `WARNINGS_<warning_key>` (with `WARNINGS_*` allowed as wildcard).
- Non-standard target keys MUST be treated as not permitted, regardless of allowlist contents, and MUST emit `override_denied` only.
...
- `NAH_OVERRIDE_WARNINGS_<WARNING_KEY>` MUST override the Host Profile warning action for that key (warn/ignore/error) at final precedence when permitted by override policy.
  - It MUST NOT be stored as an environment variable in the Launch Contract.
  - It MUST NOT modify `effective_environment`.
  - `<WARNING_KEY>` MUST be case-insensitive; implementations MUST normalize it to lowercase snake_case before lookup.
  - If permitted and an override references an unknown warning key, emit `override_invalid` (reason `unknown_warning_key`) and ignore it.
  - If permitted and the value is not `warn`, `ignore`, or `error` (case-insensitive), emit `override_invalid` (reason `invalid_value`) and ignore it.
  - If permitted and the shape is invalid, emit `override_invalid` (reason `invalid_shape`) and ignore it.
...
1. Host Profile defaults (**fill-only**)
...
- Load App Manifest, App Install Record as `install_record`, Host Profile as `profile`, process environment, and now.
...
- Then attempt to load the pinned NAK via:
  - `PinnedNakLoadResult r = load_pinned_nak(pin, manifest, profile)`
...
6. **Build effective_environment (single merged map)**
   - Start with `profile.environment` as **defaults (fill-only)**. If `profile.environment` is absent, treat it as an empty map.
   ...
   - Apply encountered overrides at final precedence ONLY when permitted by Host Profile override policy (**overwrite**). Only `NAH_OVERRIDE_ENVIRONMENT` may merge into `effective_environment`.
     - Override allow/deny and target matching MUST follow the Host Profile Override Policy and Override Allowlist Matching rules.
     ...
     - `NAH_OVERRIDE_WARNINGS_<warning_key>` MUST NOT affect `effective_environment` and MUST NOT appear in the Launch Contract environment.
...
7. **Apply warning policy overrides (process environment and overrides file)**
   - `NAH_OVERRIDE_WARNINGS_<warning_key>` MUST apply host warning policy overrides for that key only and MUST NOT affect `effective_environment`.
   - `<warning_key>` is case-insensitive and MUST be normalized to lowercase.
   - Process environment overrides: for each `NAH_OVERRIDE_WARNINGS_<warning_key>` key in lexicographic order, apply the override when permitted.
   - File overrides: the `[warnings]` table maps to `NAH_OVERRIDE_WARNINGS_<warning_key>` entries and MUST be a map of string keys to string values; entries MUST be processed in lexicographic order by derived override key.
   - If not permitted by policy, emit `override_denied` only and ignore the override.
   - If permitted but the warning key is unknown, emit `override_invalid` (reason `unknown_warning_key`) and ignore it.
   - If permitted but the value is not `warn`, `ignore`, or `error`, emit `override_invalid` (reason `invalid_value`) and ignore it.
   - If permitted but the shape is invalid, emit `override_invalid` (reason `invalid_shape`) and ignore it.
   - Any other `NAH_OVERRIDE_<X>` encountered in any source MUST be treated as not permitted and MUST emit `override_denied` only.
   - Override warning payloads MUST use `source_kind = "process_env"` with `source_ref` set to the env var name, or `source_kind = "overrides_file"` with `source_ref` set to `<file_path>` for file-level failures, `<file_path>:environment` for environment overrides, and `<file_path>:warnings` or `<file_path>:warnings.<warning_key>` for warnings as applicable.

8. **Placeholder expansion (single-pass, deterministic)**
   ...
   - Expansion MUST be applied to the following fields:
     ...
     - `profile.paths.library_prepend[]`, `profile.paths.library_append[]`
   ...
   - Expanded values MUST replace the corresponding entries in `effective_environment`. After step 8, `effective_environment` refers to the expanded environment map used for all subsequent steps and Launch Contract output.

9. **Derive capabilities and enforcement (Normative)**
   ...
   - For each derived capability, look up `profile.capabilities[key]`. If missing, emit `capability_missing` (include the capability key) and skip adding enforcement. If present, append the mapped value to `enforcement.filesystem` when `key` starts with `filesystem.` or to `enforcement.network` when `key` starts with `network.`. Preserve manifest order and do not deduplicate.
   - Selectors remain opaque; no path containment or placeholder expansion applies.

10. **Determine execution binary and arguments**
...
12. **Set library path environment key**
...
13. **Build library path list (ordered)**
    - Initialize `execution.library_paths` empty.
    - Append expanded `profile.paths.library_prepend` entries; if `profile.paths.library_prepend` is absent, treat it as an empty list. Each MUST be absolute, otherwise emit `invalid_library_path` (include `source_path` + offending value) and skip.
    ...
    - Append expanded `profile.paths.library_append` entries; if `profile.paths.library_append` is absent, treat it as an empty list. Same validation as above.

14. **Resolve asset exports**
...
15. **Finalize environment map**
    - Launch Contract environment MUST be set to the expanded `effective_environment` produced after step 8.

16. **Warnings vs CriticalError**
    ...
    - Warnings emitted by `load_pinned_nak` (including `nak_pin_invalid` or `nak_version_unsupported`) are governed by Host Profile warning policy.
...
- `action` (string): the effective action after Host Profile policy and any permitted warning overrides; one of `"warn"` or `"error"`
- `fields` (object): warning-specific fields as defined below

Warnings whose effective action is `"ignore"` MUST NOT appear in the JSON `warnings` array.
...
- For `override_invalid`, `fields.target` MUST be `OVERRIDES_FILE` for file-level parse/shape failures; otherwise `fields.target` MUST be the specific override key (`NAH_OVERRIDE_ENVIRONMENT` or `NAH_OVERRIDE_WARNINGS_<warning_key>`).
...
NAH MUST normalize capability permissions into capability keys for profile mapping:
...
```cpp
struct Capability {
    std::string key;       // Maps to profile enforcement (e.g., "filesystem.read")
...
The **capability key** maps to enforcement mechanism in Host Profile.
...
- Parse `manifest.nak_version_req` as a SemVer requirement (see SemVer Requirement (Normative)).
- If parsing fails, emit `invalid_manifest` and treat NAK as unresolved per warning policy.
- No lexicographic fallback is permitted.
...
- Apply `profile.nak.allow_versions` and `profile.nak.deny_versions`.
- If `binding_mode = "mapped"`:
  - In mapped mode, derive `min_version` and `selection_key` exactly per SemVer Parsing and Satisfaction (Normative).
  - If `profile.nak.map` contains `selection_key`, load the referenced record directly.
  - The referenced record MUST have `record.nak.id == manifest.nak_id`, and `record.nak.version` MUST satisfy `manifest.nak_version_req` and pass allow/deny rules; otherwise emit `nak_version_unsupported` and treat NAK as unresolved.
  - If no entry exists, emit `nak_version_unsupported` and treat NAK as unresolved.
...
- `nak_version_unsupported` MUST be emitted only when the pinned record was read and parsed but is incompatible with the manifest’s `nak_id` or `nak_version_req`, or is denied by Host Profile allow/deny rules; in these cases, `load_pinned_nak` MUST return `loaded == false`.
...
- If `load_pinned_nak.loaded == false`, NAH MUST omit NAK-derived library paths, omit NAK loader wiring, and omit `NAH_NAK_*` environment variables. NAH MUST emit warnings according to profile warning policy.
...
- `value`
- `source_kind` (one of `profile`, `nak_record`, `manifest`, `install_record`, `process_env`, `overrides_file`, `standard`)
- `source_path`
- `precedence_rank`

`precedence_rank` MUST be the integer rank from Precedence Rules: 1=Host Profile, 2=NAK Install Record defaults, 3=App Manifest defaults, 4=App Install Record overrides, 5=NAH standard variables, 6=process environment overrides, 7=file-based overrides.
...
- Host Profile `[warnings]` keys
- `NAH_OVERRIDE_WARNINGS_<warning_key>` overrides (normalized to lowercase)
...
enum class Warning {
    ...
    // Trust state warnings (host can map to error via profile policy)
...
Warning policy application is normative: warn = include the warning and count it as an effective warning; ignore = suppress the warning; error = include the warning and mark it as upgraded-to-error. Warning upgrades do not change composition outputs, but commands MUST exit with code 1 (see CLI Exit Codes).
compose_contract MUST NOT emit binary_not_found; a missing entrypoint is always CriticalError::ENTRYPOINT_NOT_FOUND.
...
enum class ErrorCode {
    ...
    // Profile load failures that prevent selecting an active profile file
    PROFILE_MISSING,
    PROFILE_PARSE_ERROR,
};
...
```
nah profile init <dir>
nah profile list
nah profile show [name]
nah profile set <name>
nah profile validate [name|file]
```
...
nah contract diff <id[@version]> <profileA> <profileB> [--json] [--trace]
...
- `nah contract diff` MUST compare effective values; with `--trace`, MUST include provenance diffs (source_kind/path/rank changes) for `<profileA>` vs `<profileB>`.
...
- `nah contract resolve` output MUST include registry candidates (NAK id + version + record_ref), allow/deny filtering results, semver satisfaction results, and the final selection or unresolved reason.
...
- `--fix` is optional and MUST be restricted to safe, rebuildable operations only:
  ...
  - repair `profile.current` symlink if unambiguous
...
`<kind>` MUST be one of: `profile`, `install-record`, `nak-record`, `package`, `nak-pack`, `capabilities`.
...
- `[warnings]` maps to `NAH_OVERRIDE_WARNINGS_<warning_key>` semantics (per-key override)
...
Warning overrides MUST NOT modify `effective_environment` and MUST NOT appear as env vars in the Launch Contract.
...
- 0 if completed with no effective warnings after policy
- 2 only if a Launch Contract is produced and effective warnings remain after policy (ignored warnings don't force exit 2)
- 1 if any CriticalError occurs (including `INSTALL_RECORD_INVALID`) or if a warning upgraded to error by profile policy occurs

For `--json` outputs, tools MUST still emit the JSON envelope (including `warnings`) whenever composition produces a Launch Contract. If any warning is upgraded to error by policy, the contract payload MUST still be present with `critical_error = null`, but the process MUST exit with code 1.
If both conditions occur (effective warnings remain and at least one warning is upgraded to error), the command MUST exit with code 1.
...
nah app install data-processor-2.1.0.nap --profile development
...
public:
    bool launchApplication(const std::string& app_id) {
        ...
        for (const auto& w : envelope.warnings) {
            if (w.action == "error") {
                log_error("Contract warning upgraded to error: {}", w.key);
                return false;
            }
        }

        // Apply sandboxing based on enforcement IDs
        for (const auto& enforcement : contract.enforcement.filesystem) {
            apply_filesystem_policy(enforcement);
        }

        for (const auto& enforcement : contract.enforcement.network) {
            apply_network_policy(enforcement);
        }
...
#### Override Configuration at NAK

```bash
# Ignore missing capabilities (warning policy override, not an environment variable)
NAH_OVERRIDE_WARNINGS_capability_missing=ignore nah contract show myapp

# Force development mode (merged into final environment)
NAH_OVERRIDE_ENVIRONMENT='{"NAH_NAK_MODE":"development"}' nah contract show myapp

# Use different profile temporarily
nah contract show myapp --profile development
```

#### Update Profile Without App Rebuilds
...
nah contract show myapp  # Uses updated profile
```
...
- `nah` - Canonical CLI (app/nak/profile/contract/manifest/doctor/validate/format)
...
├── include/nah/                        # public headers (manifest, contract, profile, registry, io)
├── src/
...
- Profile loading and override behavior

**Golden Tests** (regression detection):

- Fixed manifest binaries → expected decoded output
- Fixed profile/record inputs → expected launch contracts
...
- Read only local state (manifest, App Install Record, NAK Install Record, Host Profile).
...
- MUST implement the four artifacts (App Manifest, App Install Record, NAK Install Record, Host Profile) with the schemas defined in this spec.
...
NAH provides a production-grade, minimal-dependency system for native application hosting that enforces clean separation between application contracts and host policy. Through its four-artifact model (App Manifest, App Install Record, NAK Install Record, and Host Profile), NAH ensures that applications remain portable while hosts maintain complete control over security, layout, and policy.
