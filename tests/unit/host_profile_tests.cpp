...
TEST_CASE("host profile valid schema and binding_mode") {
    ...
    auto v = parse_host_profile(toml, rec);
    CHECK(v.ok);
    CHECK(rec.schema == "nah.host.profile.v1");
...
TEST_CASE("host profile missing binding_mode defaults to canonical") {
    ...
    const char* toml = R"(
        schema = "nah.host.profile.v1"
        [nak]
        # missing binding_mode - should default to canonical
    )";
    HostProfileRecord rec;
    auto v = parse_host_profile(toml, rec);
    CHECK(v.ok);
...
TEST_CASE("host profile parses allow_versions patterns") {
    ...
    auto result = parse_host_profile_full(toml, "test.toml");
    CHECK(result.ok);
