...
TEST_CASE("Composition: Permissions remain host-owned declarations") {
    AppDeclaration app;
    app.id = "com.example.permissions";
    app.version = "1.0.0";
    app.entrypoint_path = "bin/app";
    app.permissions_filesystem = {"read:host://documents/*", "custom:value"};
    app.permissions_network = {"connect:https://example.com"};

    InstallRecord install;
    install.install.instance_id = "permissions";
    install.paths.install_root = "/apps/permissions";
    install.trust.state = TrustState::Verified;
    install.trust.source = "test";
    install.trust.evaluated_at = "2026-01-01T00:00:00Z";
