...
int cmd_show(const GlobalOptions& opts, const ShowOptions& show_opts) {
    ...
    if (show_opts.target.empty()) {
        ...
        for (const auto& f : app_files) {
            if (f.size() > 5 && f.substr(f.size() - 5) == ".json") app_count++;
        }
        for (const auto& f : nak_files) {
            if (f.size() > 5 && f.substr(f.size() - 5) == ".json") nak_count++;
        }
    ...
    auto manifest_content = nah::fs::read_file(app_dir + "/nah.json");

    // If no JSON manifest, try binary manifest
    if (!manifest_content) {
        auto binary_manifest = nah::fs::read_file(app_dir + "/manifest.nah");
        if (binary_manifest) {
            // Parse binary manifest (same logic as run command)
            const uint8_t* data = reinterpret_cast<const uint8_t*>(binary_manifest->data());
            size_t len = binary_manifest->size();

            if (len >= 4 && data[0] == 'N' && data[1] == 'A' && data[2] == 'H' && data[3] == 0x02) {
                ...
                while (offset + 3 <= len) {
                    ...
                    switch (field_type) {
                        case 0x01: parsed_manifest["id"] = value; break;
                        case 0x02: parsed_manifest["version"] = value; break;
                        case 0x03: parsed_manifest["nak_id"] = value; break;
                        case 0x04: parsed_manifest["nak_version_req"] = value; break;
                        case 0x05: parsed_manifest["entrypoint"] = value; break;
                        case 0x06:
                        ...
                        case 0x07:
                            ...
                            break;
                        case 0x0A: parsed_manifest["nak_loader"] = value; break;
                    }
                }
    ...
    if (!manifest_content) {
        print_error("App manifest not found in " + app_dir, opts.json);
        return 1;
    }
